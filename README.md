...
平台需要历史数据作为“题库”。推荐从较早起始时间拉取到“当前时间”，脚本会自动按交易所实际可用历史数据返回（不存在的更早数据不会写入）。
...
```bash
NOW_UTC=$(date -u +"%Y-%m-%dT%H:%M:%SZ")
for tf in 5m 15m 30m 1h 2h 4h 6h 8h 12h 1d 3d 1w 1M; do
  python backend/app/scripts/download_history.py \
    --exchange binance \
    --pair BTC/USDT:USDT \
    --timeframe "$tf" \
...
NOW_UTC=$(date -u +"%Y-%m-%dT%H:%M:%SZ")
python backend/app/scripts/download_history.py \
  --exchange binance --pair BTC/USDT --timeframe 1h --start 2017-01-01T00:00:00Z --end "$NOW_UTC"
```
...
NOW_UTC=$(date -u +"%Y-%m-%dT%H:%M:%SZ")
python backend/app/scripts/download_history.py \
  --exchange okx --pair BTC/USDT:USDT --timeframe 1h --start 2017-01-01T00:00:00Z --end "$NOW_UTC"
```
